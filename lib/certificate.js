...
const ASSETS   = path.join(__dirname, 'cert-assets');
const TEMPLATE = path.join(ASSETS, 'template.pdf');
const SCRIPT_FONT = path.join(ASSETS, 'GreatVibes.ttf');
 ...
 * @param {Date}   [opts.endDate]    - Data fine corso (anche = completedAt)
 * @param {number} [opts.hours]      - Durata in ore
 * @param {Date}   [opts.completedAt]- Data completamento (fallback per endDate / Firenze)
 * @param {string} opts.certCode     - Codice attestato (resta nel testo per coerenza, non stampato sul layout originale)
...
async function generateCertificate(opts) {
  ...
  const endD = endDate || completedAt;
  ...
  // ── DATA FIRENZE (in basso, accanto a "Firenze, il") — Helvetica 10pt. baseline 717.3, X≈468
  drawAt(fmtDate(endD), 468.5, 716.0, helv, 10, BLACK);
