...
// Email DEDICATA al recupero password (diversa da quella di primo accesso post-pagamento)
  ...
  return resend.emails.send({
    ...
    html: `
      <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;background:#0d0d0d;color:#fff;padding:40px;">
        ...
        <h2 style="font-size:20px;margin-bottom:16px;">Reimposta la tua password</h2>
        <p style="color:#b3b3b3;line-height:1.7;margin-bottom:8px;">
          Abbiamo ricevuto una richiesta di reimpostazione della password per l'account
          <strong style="color:#fff;">${user.email}</strong>.
        </p>
        <p style="color:#b3b3b3;line-height:1.7;margin-bottom:24px;">
          Clicca il pulsante qui sotto per scegliere una nuova password. Il link è valido per 48 ore.
        </p>

        <a href="${resetUrl}" style="display:inline-block;background:#a01a36;color:#fff;padding:14px 32px;border-radius:4px;text-decoration:none;font-weight:700;font-size:13px;letter-spacing:1px;text-transform:uppercase;margin-bottom:28px;">
          Reimposta la password →
        </a>

        <div style="background:#1a1a1a;border-radius:8px;padding:16px;margin-bottom:24px;border:1px solid #333;">
          <p style="font-size:11px;color:#888;text-transform:uppercase;letter-spacing:1px;margin-bottom:6px;">Link (valido 48 ore)</p>
          <p style="font-size:12px;color:#c9a84c;word-break:break-all;font-family:monospace;">${resetUrl}</p>
        </div>

        <p style="color:#b3b3b3;font-size:13px;line-height:1.7;margin-bottom:8px;">
          <strong style="color:#fff;">Non hai richiesto tu questa modifica?</strong><br>
          Ignora questa email: la tua password resta invariata e nessuno può accedere senza il link qui sopra.
        </p>
...
// Avviso all'admin: tentativo di accesso al pannello da parte di un non-admin
async function sendAdminAccessAlertEmail(adminEmail, attempt) {
  ...
  return resend.emails.send({
    from: FROM,
    to: adminEmail,
    subject: '⚠️ Tentativo di accesso al pannello admin',
    html: `
      <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;background:#0d0d0d;color:#fff;padding:40px;">
        <h1 style="color:#a01a36;font-size:24px;margin-bottom:8px;">GrecoLatinoVivo</h1>
        <p style="color:#b3b3b3;font-size:13px;margin-bottom:28px;">Avviso di sicurezza</p>
        <h2 style="font-size:19px;margin-bottom:16px;">Tentativo di accesso non autorizzato al pannello admin</h2>
        <div style="background:#1a1a1a;border-radius:8px;padding:18px;border:1px solid #333;line-height:1.8;">
          <p style="margin:0;color:#b3b3b3;">Utente: <strong style="color:#fff;">${attempt.email || 'non autenticato'}</strong></p>
          <p style="margin:0;color:#b3b3b3;">ID utente: <strong style="color:#fff;">${attempt.userId || '—'}</strong></p>
          <p style="margin:0;color:#b3b3b3;">Quando: <strong style="color:#fff;">${when}</strong></p>
          <p style="margin:0;color:#b3b3b3;">IP: <strong style="color:#fff;">${attempt.ip || '—'}</strong></p>
        </div>
        <p style="color:#666;font-size:11px;margin-top:28px;">
          Se sei stato tu, ignora questa email. L'accesso è stato comunque bloccato.
        </p>
      </div>`,
  });
}

// ── Layout email comune GLV ──────────────────────────────────────────────
function glvEmailShell(bodyHtml) {
  return `
    <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;background:#0d0d0d;color:#fff;padding:40px;">
      <h1 style="color:#a01a36;font-size:24px;margin-bottom:8px;">GrecoLatinoVivo</h1>
      <p style="color:#b3b3b3;font-size:13px;margin-bottom:32px;">Centro Nazionale di Studi Classici</p>
      ${bodyHtml}
      <p style="color:#666;font-size:11px;margin-top:32px;">GrecoLatinoVivo · Centro Nazionale di Studi Classici</p>
    </div>`;
}
...
// Conferma prenotazione esame di certificazione
async function sendExamBookingEmail(user, session) {
  return resend.emails.send({
    from: FROM, to: user.email,
    subject: `Prenotazione confermata — ${session.title}`,
    html: glvEmailShell(`
      <h2 style="font-size:20px;margin-bottom:16px;">Prenotazione confermata</h2>
      <p style="color:#b3b3b3;line-height:1.7;">Ciao ${user.fullName || 'studente'}, la tua prenotazione all'esame di certificazione è confermata.</p>
      <div style="background:#1a1a1a;border-radius:8px;padding:20px;margin:20px 0;border:1px solid #333;">
        <p style="font-size:16px;color:#fff;margin:0 0 8px;"><strong>${session.title}</strong></p>
        <p style="color:#c9a84c;margin:0 0 4px;">${fmtDateTimeIt(session.examDate)}</p>
        <p style="color:#b3b3b3;margin:0;">${session.location || 'Online'}</p>
      </div>
      <a href="${APP_URL}/dashboard.html" style="display:inline-block;background:#a01a36;color:#fff;padding:14px 32px;border-radius:4px;text-decoration:none;font-weight:700;font-size:13px;letter-spacing:1px;text-transform:uppercase;">Vai al Portale →</a>
    `),
  });
}

// Conferma prenotazione live
async function sendLiveBookingEmail(user, session) {
  return resend.emails.send({
    from: FROM, to: user.email,
    subject: `Prenotazione confermata — ${session.title}`,
    html: glvEmailShell(`
      <h2 style="font-size:20px;margin-bottom:16px;">Sei prenotato alla live</h2>
      <p style="color:#b3b3b3;line-height:1.7;">Ciao ${user.fullName || 'studente'}, ecco i dettagli della diretta. Riceverai un promemoria 24 ore e 2 ore prima. Il link per partecipare si attiva 1 ora prima dell'inizio.</p>
      <div style="background:#1a1a1a;border-radius:8px;padding:20px;margin:20px 0;border:1px solid #333;">
        <p style="font-size:16px;color:#fff;margin:0 0 8px;"><strong>${session.title}</strong></p>
        <p style="color:#c9a84c;margin:0;">${fmtDateTimeIt(session.startAt)}</p>
      </div>
      <a href="${APP_URL}/dashboard.html" style="display:inline-block;background:#a01a36;color:#fff;padding:14px 32px;border-radius:4px;text-decoration:none;font-weight:700;font-size:13px;letter-spacing:1px;text-transform:uppercase;">Vai al Portale →</a>
    `),
  });
...
// Reminder live (24h o 2h prima). `when` = '24 ore' | '2 ore'
async function sendLiveReminderEmail(user, session, when) {
  return resend.emails.send({
    from: FROM, to: user.email,
    subject: `Tra ${when}: ${session.title}`,
    html: glvEmailShell(`
      <h2 style="font-size:20px;margin-bottom:16px;">La live inizia tra ${when}</h2>
      <div style="background:#1a1a1a;border-radius:8px;padding:20px;margin:20px 0;border:1px solid #333;">
        <p style="font-size:16px;color:#fff;margin:0 0 8px;"><strong>${session.title}</strong></p>
        <p style="color:#c9a84c;margin:0;">${fmtDateTimeIt(session.startAt)}</p>
      </div>
      <p style="color:#b3b3b3;line-height:1.7;">Il pulsante per partecipare si attiva 1 ora prima dell'inizio, nella tua area Live del portale.</p>
      <a href="${APP_URL}/dashboard.html" style="display:inline-block;background:#a01a36;color:#fff;padding:14px 32px;border-radius:4px;text-decoration:none;font-weight:700;font-size:13px;letter-spacing:1px;text-transform:uppercase;">Vai al Portale →</a>
    `),
  });
}

// Conferma email studente di una classe (docenti)
async function sendClassStudentConfirmEmail(studentEmail, studentName, className, confirmUrl) {
  return resend.emails.send({
    ...
    html: glvEmailShell(`
      <h2 style="font-size:20px;margin-bottom:16px;">Conferma iscrizione</h2>
      <p style="color:#b3b3b3;line-height:1.7;">Ciao ${studentName || ''}, sei stato iscritto alla classe <strong style="color:#fff;">${className}</strong>. Conferma il tuo indirizzo email per accedere ai test assegnati.</p>
      <a href="${confirmUrl}" style="display:inline-block;background:#a01a36;color:#fff;padding:14px 32px;border-radius:4px;text-decoration:none;font-weight:700;font-size:13px;letter-spacing:1px;text-transform:uppercase;">Conferma email →</a>
    `),
  });
}
